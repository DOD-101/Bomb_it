"""
Implements the class self.outer_instance, which extends Surface.

Status: WIP
- need to implement more draw functions
- need to implement update function
"""
...
from shared import colors, STANDARD_FONT, window_w, window_h, MENU_WIDTH, tile_size, immap, MAP_QUEUE_W, map_queue, PATH_TO_MAPS
...
class Screen(Surface):
    ...
    class Draw:
        def __init__(self, outer_instance: "Screen") -> None:
            self.outer_instance = outer_instance

        def drawGrid(self, map_image, tile_size: int, grid_start: int):
            '''Used to draw base grid before effects'''
            self.outer_instance.fill(colors["all"]["background"])
            ...
            for x in range(grid_start, map_image.size[0]*tile_size + grid_start, tile_size):
                for y in range(0, map_image.size[1]*tile_size, tile_size):
                    ...
                    pydraw.rect(self.outer_instance, pxmap[pix_x,pix_y], rect)
                    pydraw.rect(self.outer_instance, colors["game"]["grid-border"], rect, 1)
                    pix_y += 1
                pix_x += 1
                pix_y = 0

        def drawMenu(self, explode_time):
            global bomb1_btn, bomb2_btn, bomb3_btn, explode_btn, next_map_btn
            draw.rect(self.outer_instance,colors["all"]["background"], Rect(0, 0, MENU_WIDTH, window_h))
            ...
            active_bomb_ftext = active_bomb_font.render(Bomb.instances[0].nickname, True, active_bomb_font_color)
            draw.rect(self.outer_instance, colors["all"]["background"], [0, window_h-30, 190, 30])
            self.outer_instance.blit(active_bomb_ftext, (20, window_h - 20))
            # draw score text
            total_score_font = active_bomb_font
            total_score_font_color = colors["game"]["total_score-font"]
            total_score_ftext = total_score_font.render(f"Score:{calculateTotalScore()}", True, total_score_font_color)
            self.outer_instance.blit(total_score_ftext, (20, window_h - 200))

        def drawEfects(self, mouse_pos, grid_start, mouse_tile_cords):
            #hover efect
            if mouse_pos[0] > grid_start and mouse_pos[1] < immap.size[1] * tile_size:
                hover_Surface = Surface((tile_size, tile_size))
                hover_Surface.set_alpha(128)
                hover_Surface.fill(colors["game"]["grid-hover"])
                blit_x, blit_y = cordsConvert(mouse_tile_cords, True)
                self.outer_instance.blit(hover_Surface,(blit_x, blit_y))
            #clicked efects
        ...
        def drawMapSelect(self):
            ...
            mapdict = {}
            for map in os.listdir(PATH_TO_MAPS):
                file_ending = re.search(r".*(\..*)$", map).group(1)
                if not file_ending == ".png":
                    continue
                mapdict[map] = MapFrame(colors["map_select"]["map-frame"], os.path.join(PATH_TO_MAPS, map), map.removesuffix(file_ending))

            #region Map queue
            map_queue_x_buttons_dict = {}
            map_queue_element_height = 50
            def draw_map_queue_element(top_y: int, mapname: str):
                draw.rect(self.outer_instance, colors["map_select"]["queue"]["element"]["border"], [5, top_y, MAP_QUEUE_W - 10, map_queue_element_height], 1)
                ...
                self.outer_instance.blit(mapname_ftext, [10, mapname_y])
            ...
            draw.line(self.outer_instance, colors["map_select"]["queue"]["line"], [MAP_QUEUE_W, 0], [MAP_QUEUE_W, window_h], 2)
