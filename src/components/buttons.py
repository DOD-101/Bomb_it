...
import shared
from utils.utils import center, strToRGB


class Button:
    """The class used whenever adding a button somewhere in the game."""
    ...
    def __init__(
        ...
        font,
        font_color,
        border: Literal["color", "width", "radius"] = None,
    ...
    def draw(self, draw_text=True) -> None:
        ...
        """
        pydraw.rect(
            self.surface, self.color, [self.x_pos, self.y_pos, self.width, self.height]
        )
        if hasattr(self, "border"):
            pydraw.rect(
                ...
                self.border[0],
                [
                    self.x_pos - (self.border[1] / 2),
                    self.y_pos - (self.border[1] / 2),
        ...
        if draw_text:
            ...
            self.text_centered_x = self.x_pos + center(
                item_width=self.text_width,
                parent_width=self.width,
                center_direction="horizontal",
            )
            self.text_centered_y = self.y_pos + center(
                item_height=self.text_height,
    ...
    def checkMouseOver(self, mouse_pos) -> bool:
        ...
        return (
            self.x_pos <= mouse_pos[0] <= self.x_pos + self.width
...
class RoundButton(Button):
    ...
    def draw(self) -> None:
        ...
        self.text_centered_y = (
            ...
            - 1
        )  # to make text apear more centered
        self.surface.blit(self.ftext, (self.text_centered_x, self.text_centered_y))
...
class BombButton(Button):
    """
    Use this class to make any BombButtons.
    ...
    def __init__(
        ...
        height: int,
        text: str,
        bombinstance: type["Bomb"],
        insta_draw=False,
    ) -> None:
        super().__init__(
            surface,
            name,
            shared.COLORS["game"]["bomb-buttons"]["conventional"]["stage1"],
            x_pos,
            y_pos,
            width,
            height,
            ...
            shared.COLORS["game"]["bomb-buttons"]["conventional"]["font1"],
            [shared.COLORS["all"]["background"], 6, 10],
            insta_draw=insta_draw,
        )
    ...
    def onClick(self):
        """
        Sets the active_bomb and apropriate text.
        Method works but needs work
        """
    ...
    def draw(self):
        ...
        self.fprice = self.font.render(
            str(self.bombinstance.price), True, self.font_color
        )
        self.fprice_height = self.fprice.get_height()
        self.fprice_centered_y = self.y_pos + center(
            item_height=self.fprice_height,
            parent_height=self.height,
            center_direction="vertical",
        )
    ...
    def checkAndExecute(self, mouse_pos):
        """Allows for easy combining of the checkMouseOver and onClick."""
